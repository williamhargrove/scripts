...
# date 60 days ago
date_60days_ago=$(date --date="-60 days" +%F)

# delete all daily snapshots older than 60 days
daily_snapshots_to_delete=$(aws ec2 --profile=PROFILE_NAME describe-snapshots --owner-ids ACCOUNT_NUMBER --filters Name=tag:"Type",Values="daily" --query "Snapshots[?StartTime<='${date_60days_ago}'].SnapshotId" --output text)

for snap in ${daily_snapshots_to_delete}; do
  echo "Deleting daily snapshot $snap - greater than 60 days old"
  aws ec2 --profile=PROFILE_NAME delete-snapshot --snapshot-id $snap
done
